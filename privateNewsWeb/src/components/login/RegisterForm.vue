...
<template>
    <van-form @submit="onSubmit" :readonly="buttonLoading" validate-trigger="onSubmit" label-width="65px">
        <van-cell-group inset>
            <van-field v-model="username" name="username" label="用户名" placeholder="输入用户名" clearable
                @blur="(e: Event) => changeLeftIcon(username, 'username', e)" :rules="[
            ...
            <van-field v-model="password" type="password" name="password" label="密码" placeholder="输入密码" clearable
                @blur="(e: Event) => changeLeftIcon(password, 'password', e)" :rules="[{ required: true, message: '请填写密码' }, {
